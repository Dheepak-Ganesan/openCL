...
int main()
{
    ...
    vector<int> one_host(WORK_ITEMS);
    for(int i = 0 ; i <WORK_ITEMS; i++)
    {
        one_host[i] = (rand() % 21) - 10;
    }

    cl::Buffer buff_dev(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof(int) * WORK_ITEMS,one_host.data());

    cl::Program program(context,util::loadProgram("relu.cl"));
    program.build({device});

    cl::Kernel kernel(program,"eltwise_relu");
    kernel.setArg(0,buff_dev);

    queue.enqueueNDRangeKernel(kernel,cl::NullRange,cl::NDRange(WORK_ITEMS),cl::NullRange);
    queue.enqueueReadBuffer(buff_dev,CL_TRUE,0,sizeof(int) * WORK_ITEMS,one_host.data());

    cout << "Relu Results " << endl;
    for(int i = 0 ; i< WORK_ITEMS ; i++)
    {
