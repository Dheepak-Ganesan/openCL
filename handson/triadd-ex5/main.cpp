...
int main()
{
    ...
    cl::Buffer c_buffer(context,CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,sizeof(int) * WORK_ITEMS,c.data());
    cl::Buffer d_buffer(context,CL_MEM_READ_WRITE,sizeof(int) * WORK_ITEMS);
    cl::Program program(context,util::loadProgram("triadd.cl"));
    program.build({device});

    cl::Kernel kernel(program,"triadd");
