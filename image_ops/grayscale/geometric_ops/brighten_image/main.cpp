...
int main()
{
    ...
    std::string kernel_source = util::loadProgram("brighten_image.cl");
    cl::Program program_obj(context, kernel_source);
    program_obj.build({device});

    cl::Kernel kernel_add(program_obj, "brighten");
    kernel_add.setArg(0, device_image);
    kernel_add.setArg(1, WIDTH);

    CommandQueue.enqueueNDRangeKernel(kernel_add, cl::NullRange, cl::NDRange(HEIGHT,WIDTH));
    CommandQueue.finish();
    ...
    // Open output file
    std::ofstream out_file("brightened_image.txt");
    if (!out_file.is_open()) {
        std::cerr << "Error: Could not create brightened_image.txt\n";
        return -1;
    }
    ...
    for (int row = 0; row < HEIGHT; row++) {
        for (int col = 0; col < WIDTH; col++) {
            out_file << input_image[row * WIDTH + col];
            if (col < WIDTH - 1)
                out_file <<  " ";
        }
        out_file << "\n";
    ...
    out_file.close();
    std::cout << "Brightened image saved to brightened_image.txt\n";
