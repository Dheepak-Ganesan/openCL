...
int main()
{
    std::ifstream file("vertical_flip.txt");
    if (!file.is_open()) {
        std::cerr << "Error: Could not open vertical_flip.txt\n";
        return -1;
    }
    ...
    // Save the image
    cv::imwrite("vertical_flip.png", img);

    std::cout << "Image saved as vertical_flip.png\n";
    return 0;
}
