...
int main()
{
    std::ifstream file("threshold_image.txt");
    if (!file.is_open()) {
        std::cerr << "Error: Could not open threshold_image.txt\n";
        return -1;
    }
    ...
    // Save the image
    cv::imwrite("threshold_image.png", img);

    std::cout << "Image saved as threshold_image.png\n";
    return 0;
}
