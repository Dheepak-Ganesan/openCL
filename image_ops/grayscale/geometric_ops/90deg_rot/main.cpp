...
int main()
{
    ...
    CommandQueue.enqueueNDRangeKernel(kernel_90deg_rot, cl::NullRange, cl::NDRange(HEIGHT,WIDTH));
    CommandQueue.finish();
    ...
    // Open output file
    std::ofstream out_file("90deg_rot.txt");
    if (!out_file.is_open()) {
        std::cerr << "Error: Could not create 90deg_rot.txt\n";
        return -1;
    }
    ...
    out_file.close();
    std::cout << "Output image saved to 90deg_rot.txt\n";
