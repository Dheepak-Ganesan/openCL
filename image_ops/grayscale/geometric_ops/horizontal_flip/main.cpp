...
int main()
{
    ...
    CommandQueue.enqueueNDRangeKernel(kernel_horizontal_flip, cl::NullRange, cl::NDRange(HEIGHT,WIDTH));
    CommandQueue.finish();
    ...
    // Open output file
    std::ofstream out_file("horizontal_flip.txt");
    if (!out_file.is_open()) {
        std::cerr << "Error: Could not create horizontal_flip.txt\n";
        return -1;
    }
    ...
    out_file.close();
    std::cout << "Output image saved to horizontal_flip.txt\n";
