...
int main()
{
    std::ifstream file("horizontal_flip.txt");
    if (!file.is_open()) {
        std::cerr << "Error: Could not open horizontal_flip.txt\n";
        return -1;
    }
    ...
    // Save the image
    cv::imwrite("horizontal_flip.png", img);

    std::cout << "Image saved as horizontal_flip.png\n";
    return 0;
}
