...
int main()
{
    ...
    std::string kernel_source = util::loadProgram("counter_90deg_rot.cl");
    ...
    CommandQueue.enqueueNDRangeKernel(kernel_counter_90deg_rot, cl::NullRange, cl::NDRange(HEIGHT,WIDTH));
    CommandQueue.finish();
    ...
    // Open output file
    std::ofstream out_file("counter_90deg_rot.txt");
    if (!out_file.is_open()) {
        std::cerr << "Error: Could not create counter_90deg_rot.txt\n";
        return -1;
    }
    ...
    out_file.close();
    std::cout << "Output image saved to counter_90deg_rot.txt\n";
