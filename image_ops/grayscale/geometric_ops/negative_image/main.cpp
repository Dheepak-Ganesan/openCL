...
int main()
{
    ...
    std::string kernel_source = util::loadProgram("neg_img.cl");
    cl::Program program_obj(context, kernel_source);
    program_obj.build({device});

    cl::Kernel kernel_add(program_obj, "neg_img");
    ...
    // Open output file
    std::ofstream out_file("negative_image.txt");
    if (!out_file.is_open()) {
        std::cerr << "Error: Could not create negative_image.txt\n";
        return -1;
    }
    ...
    out_file.close();
    std::cout << "Negative image saved to negative_image.txt\n";
