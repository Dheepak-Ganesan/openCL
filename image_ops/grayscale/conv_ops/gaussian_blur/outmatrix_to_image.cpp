...
int main()
{
    std::ifstream file("gaussian_filter.txt");
    if (!file.is_open()) {
        std::cerr << "Error: Could not open gaussian_filter.txt\n";
        return -1;
    }
    ...
    // Save the image
    cv::imwrite("gaussian_filter.png", img);

    std::cout << "Image saved as gaussian_filter.png\n";
    return 0;
}
