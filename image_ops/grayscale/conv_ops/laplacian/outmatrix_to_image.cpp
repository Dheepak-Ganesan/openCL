...
int main()
{
    std::ifstream file("laplacian_filter.txt");
    if (!file.is_open()) {
        std::cerr << "Error: Could not open laplacian_filter.txt\n";
        return -1;
    }
    ...
    // Save the image
    cv::imwrite("laplacian_filter.png", img);

    std::cout << "Image saved as laplacian_filter.png\n";
    return 0;
}
