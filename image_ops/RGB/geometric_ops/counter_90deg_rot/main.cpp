...
int main()
{
    ...
    vector<int> output_image(GLOBAL_DIM);
    ...
    for(int row=0; row<5; row++)
    {
        for(int col=0; col<5; col++)
            cout << input_image[0 * HEIGHT * WIDTH + row * WIDTH + col] << "\t";
        cout << "\n";
    }
    ...
    string kernel_source = util::loadProgram("counter_90deg_rot.cl");
    cl::Program program_obj(context, kernel_source);
    program_obj.build({device});

    cl::Kernel kernel_counter_90deg_rot(program_obj, "rotate_90_counterClkWise");
    kernel_counter_90deg_rot.setArg(0, device_image);
    kernel_counter_90deg_rot.setArg(1, device_output);
    kernel_counter_90deg_rot.setArg(2, HEIGHT);
    kernel_counter_90deg_rot.setArg(3, WIDTH);
    kernel_counter_90deg_rot.setArg(4, CHANNELS);

    CommandQueue.enqueueNDRangeKernel(kernel_counter_90deg_rot, cl::NullRange, cl::NDRange(HEIGHT, WIDTH, CHANNELS));
    CommandQueue.finish();
    ...
    {
        for(int col=0; col<5; col++)
            cout << output_image[0 * HEIGHT * WIDTH + row * WIDTH + col] << "\t";
        cout << "\n";
    }

    ofstream out_file("counter_90deg_rot_3d.txt");
    ...
    out_file.close();
    cout << "Output 3D image saved to counter_90deg_rot_3d.txt\n";
}
