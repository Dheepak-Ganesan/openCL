...
int main()
{
   ...
   file >> CHANNELS >> HEIGHT >> WIDTH;
    ...
    kernel_vertical_flip.setArg(4, CHANNELS);

    CommandQueue.enqueueNDRangeKernel(kernel_vertical_flip, cl::NullRange, cl::NDRange(HEIGHT, WIDTH, CHANNELS));
    CommandQueue.finish();
    ...
    std::ofstream out_file("vertical_flip_3d.txt");
    ...
    for(int c = 0; c < CHANNELS; c++)
        for(int row = 0; row < HEIGHT; row++)
        {
            for(int col = 0; col < WIDTH; col++)
            {
                out_file << output_image[c * WIDTH * HEIGHT + row * WIDTH + col];
    ...
    out_file.close();
    std::cout << "Output 3D image saved to vertical_flip_3d.txt\n";
}
