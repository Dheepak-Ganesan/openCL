...
int main()
{
    ...
    // Load 3D image matrix from TXT file
    ...
    std::string kernel_source = util::loadProgram("brighten_img.cl");
    ...
    std::ofstream out_file("brighten_img_3d.txt");
    if (!out_file.is_open()) {
        std::cerr << "Error: Could not create brighten_img_3d.txt\n";
        return -1;
    }
    ...
    out_file.close();
    std::cout << "Output 3D image saved to brighten_img_3d.txt\n";
}
