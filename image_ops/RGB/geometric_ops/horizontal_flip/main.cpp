...
int main()
{
    ...
    std::string kernel_source = util::loadProgram("horizontal_flip.cl");
    cl::Program program_obj(context, kernel_source);
    program_obj.build({device});

    cl::Kernel kernel_horizontal_flip(program_obj, "horizontal_flip");
    kernel_horizontal_flip.setArg(0, device_image);
    kernel_horizontal_flip.setArg(1, device_output);
    kernel_horizontal_flip.setArg(2, WIDTH);
    kernel_horizontal_flip.setArg(3, HEIGHT);
    kernel_horizontal_flip.setArg(4, CHANNELS);

    CommandQueue.enqueueNDRangeKernel(kernel_horizontal_flip, cl::NullRange, cl::NDRange(HEIGHT, WIDTH, CHANNELS));
    CommandQueue.finish();
    ...
    std::ofstream out_file("horizontal_flip_3d.txt");
    ...
    out_file.close();
    std::cout << "Output 3D image saved to horizontal_flip_3d.txt\n";
}
